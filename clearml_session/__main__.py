...
try:
    Session.add_client(
        __package__.partition(".")[0].replace("_", "-"), __version__
    )  # noqa
except Exception:
    pass
...
def _get_config_section_name():
    ...
    try:
        ...
        from interactive_session_task import (  # noqa
            config_object_section_bash_init,
            config_object_section_ssh,
            config_section_name,
        )
...
def _check_ssh_executable():
    ...
    if sys.platform == "win32" and getattr(sys, "winver", "").endswith("-32"):
        raise ValueError(
            "Python 32-bit version detected. Only Python 64-bit is supported!"
        )
    ...
    try:
        if sys.platform == "win32":
            ssh_exec = (
                subprocess.check_output("where ssh.exe".split())
                .decode("utf-8")
                .split("\n")[0]
                .strip()
            )
            ...
            ssh_exec = (
                subprocess.check_output("which ssh".split())
                .decode("utf-8")
                .split("\n")[0]
                .strip()
            )
...
def _get_available_ports(list_initial_ports):
    ...
    for p in list_initial_ports:
        port = next(
            i
            for i in range(p, 65000)
            if i not in available_ports
            and (
                (used_ports is not None and i not in used_ports)
                or (used_ports is None and _check_available_port(i))
            )
        )
...
def create_base_task(
    state, project_name=None, task_name=None, continue_task_id=None, project_id=None
):
    if continue_task_id:
        task = Task.clone(
    ...
    base_script_file = os.path.abspath(
        os.path.join(__file__, "..", "interactive_session_task.py")
    )
    with open(base_script_file, "rt") as f:
        task_script["diff"] += "\n\n" + f.read()
    ...
    task_script["requirements"] = {
        "pip": "\n".join(
            ["clearml>=1.1.5"]
            + (
                ["jupyter", "jupyterlab", "jupyterlab_git", "traitlets"]
...
def create_debugging_task(state, debug_task_id, task_name=None, task_project_id=None):
    ...
    if not debug_task.data.script.repository:
        raise ValueError(
            "Debugging task has no git repository, single script debugging is not supported."
        )

    task = Task.clone(
        source_task=debug_task_id,
        parent=debug_task_id,
        name=task_name,
        ...
        try:
            ...
            if state.get("store_workspace") and task.artifacts:
                if continue_session_id and continue_session_id == t.id:
                    print(
                        "Restoring workspace from previous session id={} [{}]".format(
                    ...
                    if not state.get("yes"):
                        choice = input(
                            "Restore workspace from session id={} '{}' @ {} [Y]/n? ".format(
                                t.id,
                                task.name,
                                str(task.data.last_update).split(".")[0],
                            )
                        )
                    ...
                    print(
                        "Restoring workspace from previous session id={}".format(t.id)
                    )
                    state["continue_session"] = t.id
                    break
        except Exception as ex:
            logging.getLogger().warning(
                "Failed retrieving old session {}:".format(t.id, ex)
            )
...
def _get_running_tasks(client, prev_task_id):
    ...
    if prev_task_id and prev_task_id not in (t[0] for t in tasks_id_created):
        ...
        if prev_tasks:
            tasks_id_created += [
                (prev_tasks[0].id, prev_tasks[0].created, prev_tasks[0].parent)
            ]
...
def _get_user_id(client):
    if not client:
        client = APIClient()
    res = client.session.send_request(
        service="users", action="get_current_user", async_enable=False
    )
    assert res.ok
...
def get_project_id(project_name):
    ...
    if project_name:
        ...
        if not project_id:
            logging.getLogger().warning(
                "could not locate project by the named '{}'".format(project_name)
            )
    return project_id
...
def get_user_inputs(args, parser, state, client):
    ...
    if "base_task_id" in user_args and getattr(args, "base_task_id", None) != state.get(
        "base_task_id"
    ):
        print(
            "New base_task_id '{}', clearing previous packages & init_script".format(
        ...
        if a in default_needed_args and not state.get(a):
            if assume_yes:
                raise NonInteractiveError(
                    "Using `--yes` but could not locate previously used value of '{}'".format(
                        a
                    )
                )
            # noinspection PyProtectedMember
    ...
    if not state.get("password") or state.get("randomize") is not False:
        state["password"] = hashlib.sha256(
            "seed me {} {}".format(uuid4(), time()).encode()
        ).hexdigest()
    ...
    if args.requirements:
        state["packages"] = (args.packages or []) + [
            p.strip()
            for p in args.requirements.readlines()
    ...
    if assume_yes:
        if ask_queues:
            raise NonInteractiveError(
                "Using `--yes` but no queue provided or previously used"
            )
        print("Using previous queue (resource) '{}'".format(state["queue"]))
    elif state.get("queue"):
        choice = input(
            "Use previous queue (resource) '{}' [Y]/n? ".format(state["queue"])
        )
        if str(choice).strip().lower() in ("n", "no"):
            ask_queues = True
    ...
    if ask_queues:
        ...
        if not state.get("queue_include_tag") and not state.get("queue_excluded_tag"):
            ...
            queues = sorted(
                [q.name for q in client.queues.get_all(system_tags=[str(system_tag)])]
            )
            ...
            queues = sorted(
                [
                    ...
                    for q in client.queues.get_all(
                        system_tags=[
                            "-{}".format(t)
                            for t in state.get("queue_excluded_tag", ["internal"])
                        ]
    ...
    print(
        "\nInteractive session config:\n{}\n".format(
            json.dumps(
                {k: v for k, v in state.items() if not str(k).startswith("__")},
                indent=4,
                sort_keys=True,
            )
        )
...
def clone_task(state, project_id=None):
    ...
    project_id = project_id or (
        get_project_id(project_name=state.get("project"))
    ...
    if state.get("debugging_session"):
        print(
            "Starting new debugging session to {}".format(
    ...
    if runtime_prop_support:
        ...
        runtime_properties["_user_key"] = str(
            config_obj.get("api.credentials.access_key")
        )
        runtime_properties["_user_secret"] = config_obj.get(
    ...
    if not runtime_prop_support:
        ...
        task_params["{}/user_key".format(section)] = config_obj.get(
            "api.credentials.access_key"
        )
        task_params["{}/user_secret".format(section)] = config_obj.get(
            "api.credentials.secret_key"
        )
    ...
    task_params["{}/vscode_extensions".format(section)] = (
        state.get("vscode_extensions") or ""
    )
    ...
    if docker:
        task_params["{}/default_docker".format(section)] = docker.replace(
            "--network host", ""
        ).strip()
        ...
        task.set_base_docker(
            ...
            + (
                ...
                and "--network host" not in docker
                else ""
            )
        )
    # set the bash init script
    if state.get("init_script") is not None and (
        not new_task or state.get("init_script").strip()
    ):
        # noinspection PyProtectedMember
        task._set_configuration(
            name=init_section,
            config_type="bash",
            config_text=state.get("init_script") or "",
        )
    ...
    if state.get("packages"):
        ...
        packages = [
            p for p in state["packages"] if p.strip() and not p.strip().startswith("#")
        ]
        packages_id = set(
            reduce(lambda a, b: a.split(b)[0], "#;@=~<>", p).strip() for p in packages
        )
    ...
    if state.get("continue_session") and state.get("store_workspace"):
        print(
            "Restoring remote workspace from [{}] into {}".format(
...
def wait_for_machine(state, task, only_wait_for_ssh=False):
    ...
    last_message = None
    stopped_counter = 0
    while last_status != "in_progress" and last_status in (
        None,
        "created",
        "queued",
        "unknown",
        ...
        if last_status != status or last_message != message:
            ...
            print(
                "Status [{}]{} {}".format(
                    status, " - {}".format(last_status) if last_status else "", message
                )
            )
        last_status = status
    ...
    section, _, _ = _get_config_section_name()
    jupyterlab = (
        task.get_parameter("{}/jupyterlab".format(section))
        or task.get_parameter("General/jupyterlab")
        or ""
    )
    state["jupyter_lab"] = jupyterlab.strip().lower() != "false"
    vscode_server = (
        task.get_parameter("{}/vscode_server".format(section))
        or task.get_parameter("General/vscode_server")
        or ""
    )
    ...
    period_counter = 0
    while (
        any(bool(not task.get_parameter(p)) for p in wait_properties)
        and task.get_status() == "in_progress"
    ):
        ...
        if last_lines != lines:
            ...
            period_counter += 1
            print(
                ("" if state.get("verbose") else "\r") + "." * period_counter,
                end="",
                flush=True,
            )
    ...
    if task.get_status() != "in_progress":
        ...
        raise ValueError(
            "Remote setup failed (status={}) see details: {}".format(
...
def find_preferred_public_key():
    possible_keys = [
        os.path.join(os.path.expanduser("~"), ".ssh", "id_ed25519.pub"),
        os.path.join(os.path.expanduser("~"), ".ssh", "id_rsa.pub"),
        os.path.join(os.path.expanduser("~"), ".ssh", "id_ecdsa.pub"),
        os.path.join(os.path.expanduser("~"), ".ssh", "id_dsa.pub"),
    ]

    # Look inside ssh config file for IdentityFile
    ssh_config = os.path.join(os.path.expanduser("~"), ".ssh", "config")
    if os.path.isfile(ssh_config):
        with open(ssh_config, "r", encoding="utf-8") as f:
            for line in f:
                match = re.search(r"^\s*IdentityFile\s+(.+\.pub)\s*$", line)
                if match:
                    expanded = os.path.expanduser(match.group(1))
                    possible_keys.insert(0, expanded)

    for key_file in possible_keys:
        if os.path.isfile(os.path.abspath(key_file)):
            return os.path.abspath(key_file)
    return None


def _ssh_connect_and_auth(
    command, args, ssh_password=None, debug=False, ignore_fingerprint_verification=False
):
    # Add verbose flag for SSH
    args.insert(0, "-v")
    used_password_auth = None
    fd = sys.stdout if debug else StringIO()
    child = pexpect.spawn(
        command=command, args=args, logfile=fd, timeout=20, encoding="utf-8"
    )
    i = child.expect(
        [
            r"Authenticated to .* using \"publickey\"",  # Detect pubkey success
            r"(?i)password:",  # Detect password prompt
            r"\(yes\/no",  # Detect fingerprint verification
            r"(?i)permission denied",  # Detect permission denied
            pexpect.EOF,  # Detect EOF
        ]
    )
    if i == 0:
        ...
        try:
            ...
            print(
                "{}Error: incorrect password".format(fd.read() + "\n" if debug else "")
            )
            ssh_password = getpass("Please enter password manually: ")
            child.sendline(ssh_password)
            child.expect([r"(?i)password:"], timeout=5)
            print(
                "{}Error: incorrect user input password".format(
                    fd.read() + "\n" if debug else ""
                )
            )
        ...
        if "UserKnownHostsFile" in args:
            print(
                "{}! Secure fingerprint of remote server failed to verify!".format(
        ...
        child.close()
        print(
            "{}Error: Permission denied. Unable to establish SSH connection.".format(
                fd.read() + "\n" if debug else ""
            )
        )
        raise Exception("Permission denied")
    return child, fd, used_password_auth


def copy_public_key_to_server(
    key_file,
    host_file_args,
    username,
    remote_address,
    ...
    ssh_password,
    debug=False,
    ignore_fingerprint_verification=False,
):
    print(
        f"Copying public key from {key_file} to remote server for public-key authentication"
    )
    command = None
    child = None
    fd = None
    ...
    try:
        ...
        child, fd, _ = _ssh_connect_and_auth(
            command,
            args,
            ssh_password=ssh_password,
            debug=debug,
            ignore_fingerprint_verification=ignore_fingerprint_verification,
        )
        ...
        print("Public key successfully copied to remote server\n")

    except Exception as ex:
        if debug:
            output = fd.read() + "\n" if fd else ""
            print(
                "{}Error: Failed copying public key [{}] to {}: {}".format(
                    output, command, args, ex
                )
            )
        else:
            print(
                "Error: Failed copying public key to {} [{}]\n"
                "       for additional details re-run with --verbose".format(
                    remote_address, type(ex)
                )
            )
...
def start_ssh_tunnel(
    ...
    print(
        "Starting SSH tunnel to {}@{}, port {}".format(
            username, remote_address, ssh_port
        )
    )
    child = None
    used_password_auth = None
    args = [
        "-C",
        "{}@{}".format(username, remote_address),
        "-p",
        "{}".format(ssh_port),
        "-o",
        "Compression=yes",
        ...
        if Session.check_min_api_version("2.20"):
            ...
            try:
                res = task.session.send_request(
                    ...
                    "types=remote_session_ssh_server",
                ).json()
                found_server_ssh_fingerprint = json.loads(
                    res["data"]["vaults"][-1]["data"]
                )
            except Exception:
                pass
    ...
    if found_server_ssh_fingerprint:
        ...
        for k in found_server_ssh_fingerprint:
            if k.endswith("__pub"):
                known_host_lines += "{} {}\n".format(
                    remote_address, found_server_ssh_fingerprint[k]
                )
    ...
    if known_host_lines:
        ...
        host_file_args = ["-o", "UserKnownHostsFile={}".format(temp_host_file.name)]
    else:
        host_file_args = [
            "-o",
    ...
    try:
        command = _check_ssh_executable()
        child, fd, used_password_auth = _ssh_connect_and_auth(
            command,
            args,
            ssh_password=ssh_password,
            debug=debug,
        ...
        if debug:
            output = fd.read() + "\n" if fd else ""
            print(
                "{}Error: running local SSH client [{}] failed connecting to {}: {}".format(
                    output, command, args, ex
                )
            )
        else:
            print(
                "Error: running local SSH client failed connecting to {} [{}]\n"
                "       for additional details re-run with --verbose".format(
                    remote_address, type(ex)
                )
            )
    ...
    if child:
        child.logfile = None
    return child, used_password_auth, host_file_args
...
def monitor_ssh_tunnel(state, task, ssh_setup_completed_callback=None):
    ...
    ssh_process = None
    sleep_period = 3
    ssh_port = jupyter_token = jupyter_port = internal_ssh_port = ssh_password = (
        remote_address
    ) = None
    vscode_port = None
    ...
    try:
        while task.get_status() == "in_progress":
            if not all(
                ...
                if Session.check_min_api_version("2.13"):
                    ...
                    ssh_password = runtime_prop.get("_ssh_password") or state.get(
                        "password", ""
                    )
                    ...
                    section = (
                        "General"
                        if "General/ssh_server" in task_parameters
                        else default_section
                    )
                    ssh_password = task_parameters.get(
                ...
                remote_address = (
                    remote_address
                    or task_parameters.get("properties/k8s-gateway-address")
                    or task_parameters.get("properties/external_address")
                )

                internal_ssh_port = internal_ssh_port or task_parameters.get(
                    "properties/internal_ssh_port"
                )
                ...
                ssh_port = (
                    ssh_port
                    or task_parameters.get("properties/k8s-pod-port")
                    or task_parameters.get("properties/external_ssh_port")
                    or internal_ssh_port
                )

                if state.get("keepalive"):
                    internal_ssh_port = (
                        task_parameters.get("properties/internal_stable_ssh_port")
                        or internal_ssh_port
                    )
            ...
            if connect_state.get("reconnect"):
                ...
                try:
                    ssh_process.close(
                        **({"force": True} if sys.platform != "win32" else {})
                    )
                    ssh_process = None
                except Exception:
                    pass
            ...
            short_console_msg = 'Enter "r" ("reconnect"), `s` ("shell"), `Ctrl-C` ("quit") or "Shutdown"'

            if not ssh_process or not ssh_process.isalive():
                ignore_fingerprint_verification = state.get("disable_fingerprint_check")
                username = state.get("username") or "root"

                ssh_process, used_password_auth, host_file_args = start_ssh_tunnel(
                    username,
                    remote_address,
                    ssh_port,
                    ssh_password,
                ...
                if ssh_process and ssh_process.isalive():
                    key_file = find_preferred_public_key()
                    if key_file and used_password_auth:
                        copy_public_key_to_server(
                            key_file=key_file,
                            host_file_args=host_file_args,
                            username=username,
                    ...
                    if jupyter_port:
                        msg += "\nJupyter Lab URL: http://localhost:{local_jupyter_port}/?token={jupyter_token}".format(
                            local_jupyter_port=local_jupyter_port,
                            jupyter_token=jupyter_token.rstrip(),
                        )
                        if state.get("user_folder"):
                            msg += "&file-browser-path={}".format(
                                state.get("user_folder")
                            )
                    if vscode_port:
                    ...
                    if ssh_setup_completed_callback and callable(
                    ...
                    logging.getLogger().warning(
                        "SSH tunneling failed, retrying in {} seconds".format(3)
                    )
                    sleep(3.0)
            ...
            if user_input is None:
                ...
                try:
                    ...
                    open_ports = [
                        p.laddr.port
                        for p in proc.connections(kind="tcp4")
                        if p.status == "LISTEN"
                    ]
                    remote_ports = [
                        p.raddr.port
                    ...
                    if (
                        (
                            state.get("jupyter_lab")
                            and int(local_jupyter_port_) not in open_ports
                        )
                        or int(local_ssh_port_) not in open_ports
                        or int(ssh_port) not in remote_ports
                    ):
        ...
        print(
            "Remote session shutdown" if shutdown else "Remote session still running!"
        )
...
class CliCommands:
    ...
    def list_sessions(cls, args):
        ...
        for i, t in enumerate(session_tasks):
            # noinspection PyProtectedMember
            pname = (
                project_names.get(t.project, Task._get_project_name(t.project))
                if t.project
                else ""
            )
            print(
                "{}] id={} name='{}' tags={} project='{}'".format(
                    i, t.id, t.name, t.tags, pname
                )
            )
...
def setup_parser(parser):
    ...
    subparsers = parser.add_subparsers(
        help="ClearML session control commands", dest="command"
    )
    ...
    parser_info = subparsers.add_parser(
        "info", help="Detailed information on specific session"
    )
    ...
    parser_shutdown = subparsers.add_parser(
        "shutdown", help="Shutdown specific session"
    )
...
def cli():
    ...
    if args.shutdown is not None:
        task = _get_previous_session(
            ...
            args,
            state,
            task_id=args.shutdown,
            verb="Shutting down",
            question_verb="Shut down",
            ask_for_explicit_id=True,
        )
        if not task:
    ...
    if args.upload_files:
        if not Path(args.upload_files).expanduser().exists():
            print(
                "Requested file/folder `{}` does not exist, exiting".format(
                    args.upload_files
                )
            )
            return 1
    ...
    if task:
        ...
        if state.get("store_workspace") or state.get("continue_session"):
            ...
            delete_old_tasks_callback = partial(
    ...
    # launch ssh tunnel
    monitor_ssh_tunnel(
        state, task, ssh_setup_completed_callback=delete_old_tasks_callback
    )
...
def _get_previous_session(
    ...
    task_id=None,
    verb="Connecting to",
    question_verb="Connect to",
    ask_for_explicit_id=False,
):
    assume_yes = args.yes
    if task_id:
        ...
        if status == "in_progress":
            if not args.debugging_session or task.parent == args.debugging_session:
                if assume_yes or not ask_for_explicit_id:
                    ...
                    return task
                choice = input(
                    "{} active session id={} [Y]/n? ".format(question_verb, task_id)
                )
                if str(choice).strip().lower() in ("y", "yes"):
                    return task
                return None
        raise ValueError(
            "Could not connect to requested session id={} - status '{}'".format(
    ...
    if args.debugging_session:
        running_task_ids_created = [
            t for t in running_task_ids_created if t[2] == args.debugging_session
        ]
        if not running_task_ids_created:
            print(
                "No active task={} debugging session found".format(
                    args.debugging_session
                )
            )
            return None
    ...
    try:
        ...
        default_i = next(
            i
            for i, (tid, _, _) in enumerate(running_task_ids_created)
            if prev_task_id == tid
        )
    except StopIteration:
        default_i = None
    ...
    session_list = "\n".join(
        "{}{}] {} id={}".format(
            i, "*" if i == default_i else "", dt.strftime("%Y-%m-%d %H:%M:%S"), tid
        )
                ...
                choice = input(
                    ...
                    + "\n{} session [{}] or 'N' to skip: ".format(
                        ...
                        (
                            "0"
                            if len(running_task_ids_created) <= 1
                            else "0-{}".format(len(running_task_ids_created) - 1)
                        ),
                    )
                )
